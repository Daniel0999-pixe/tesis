...
file_path_prueba = r'C:\xampp\htdocs\tesis2\dataframe\modelo4\prueba\pruebaM4.xlsx'
data_prueba = pd.read_excel(file_path_prueba)

# Cargar los LabelEncoders utilizados para codificar las etiquetas
label_encoder_materia = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\label\label_encoder_materiaM4.joblib')
label_encoder_ciclo = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\label\label_encoder_cicloM4.joblib')
label_encoder_carrera = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\label\label_encoder_carreraM4.joblib')
label_encoder_curso = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\label\label_encoder_cursoM4.joblib')

# Preprocesamiento de datos: Codificar variables categóricas
data_prueba['MATERIA'] = label_encoder_materia.transform(data_prueba['MATERIA'])
data_prueba['CARRERA'] = label_encoder_carrera.transform(data_prueba['CARRERA'])

# Limpia la columna 'CICLO'
data_prueba['CICLO'] = data_prueba['CICLO'].str.strip()

# Verifica si hay valores desconocidos para el codificador
unknown_values = set(data_prueba['CICLO']) - set(label_encoder_ciclo.classes_)
if unknown_values:
    print(f"Valores desconocidos encontrados en 'CICLO': {unknown_values}")

# Intenta transformar con manejo de errores
try:
    data_prueba['CICLO'] = label_encoder_ciclo.transform(data_prueba['CICLO'])
except ValueError as e:
    print(f"Error al transformar 'CICLO': {e}")

# Verificar si hay valores faltantes
if data_prueba.isnull().sum().any():
  print("Hay valores faltantes en el DataFrame. Por favor, maneja estos valores antes de continuar.")

# Definir características para la predicción
X_prueba = data_prueba[['MATERIA', 'CICLO', 'CARRERA','agrtic', 'agrdic', 'agrped' ]]

# Cargar los modelos entrenados
model_knn = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\modelos\K-Nearest_NeighborsM4.joblib')
model_rf = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\modelos\Random_ForestM4.joblib')
model_dt = joblib.load(r'C:\xampp\htdocs\tesis2\algoritmos\entrenamiento4\modelos\Decision_TreeM4.joblib')
...
metrics_file_path = r'C:\xampp\htdocs\tesis2\dataframe\modelo4\entrenamiento\resultados_metricas_modelosM4.csv'
...
for index, row in data_prueba.iterrows():
    for model_name in results.keys():
        resultados_finales.append({
            ...
            'DOCENTE': row['DOCENTE'],
            'MATERIA': label_encoder_materia.inverse_transform([row['MATERIA']])[0],  # Convertir a texto original
            'AREA': row['AREA'],
            'CARRERA': label_encoder_carrera.inverse_transform([row['CARRERA']])[0],  # Convertir a texto original
            'CICLO': label_encoder_ciclo.inverse_transform([row['CICLO']])[0],  # Convertir a texto original
            ...
            'agrped': row['agrped'],
            'total_proceso': row['total_proceso'],
            'ESTADO': row['ESTADO'],
...
output_file_path_predictions_all_models = r'C:\xampp\htdocs\tesis2\dataframe\modelo4\prueba\resultados_predicciones_todos_modelosM4.xlsx'
...
data_prueba_best_model['MATERIA'] = label_encoder_materia.inverse_transform(data_prueba_best_model['MATERIA'])
data_prueba_best_model['CICLO'] = label_encoder_carrera.inverse_transform(data_prueba_best_model['CARRERA'])
data_prueba_best_model['CARRERA'] = label_encoder_carrera.inverse_transform(data_prueba_best_model['CARRERA'])

# Guardar resultados del mejor modelo en un nuevo archivo Excel
output_file_path_best_model_predictions = r'C:\xampp\htdocs\tesis2\dataframe\modelo4\prueba\resultados_prediccionesM4.xlsx'
...
for model_name in results.keys():
    ...
    model_predictions_df['CARRERA'] = label_encoder_carrera.inverse_transform(model_predictions_df['CARRERA'])
    ...
    output_file_path_model_predictions = f'C:\\xampp\\htdocs\\tesis2\\dataframe\\modelo4\\prueba\\resultados_predicciones_{model_name}M4.xlsx'
...
for model_name in results.keys():
    ...
    plt.title(f'Distribución de Cursos Recomendados - {model_name}', loc='center')
    ...
    output_image_path_cursos = f'C:\\xampp\\htdocs\\tesis2\\imagenes\\modelo4\\prueba\\distribucion_cursos_recomendados_{model_name}M4.png'
